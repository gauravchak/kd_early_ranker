...
class KDAuxMultitaskEarlyRanker(nn.Module):
    def __init__(
        ...
        self.mtml = nn.Sequential(
            nn.Linear(2 * embedding_dim, num_user_tasks)
        )
    ...
    def forward(
        ...
        translated_task_logits = task_logits * self.weight + self.bias

        # Concatenate task_logits and translated_task_logits
        logits = torch.cat([task_logits, translated_task_logits], dim=-1)
